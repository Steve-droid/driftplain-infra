...
| In-cluster backup CronJob (Roles Anywhere leaf) | home child `backup`, CronJob `23 * * * *` | **Suspended** (image digest + sealed owner credentials + sessions enablement) | gitops `charts/home-server-backup/values.yaml`; image in `backup-image/` |
...
| S3 retention lifecycle (hourly 1 d, daily 30 d, noncurrent/delete-marker cleanup) | infra `bootstrap/` | **Planned, not applied** | `hm5-backup-evidence.json` → `retention_plan` |
...
Build and publish `backup-image/` by digest (reviewed step), enable Roles Anywhere sessions
(`home-server/identity`: `home_server_sessions_enabled=true`, fresh locked plan, approval), seal
`home-server-backup-db-owner` for the `home-server-backups` namespace, set `enabled: true` and
`image.digest` in the backup chart values, then set `enabled: false` in the Mac `schedule.json`
only after the first in-cluster hourly object is verified with a `disposable-target` restore.
